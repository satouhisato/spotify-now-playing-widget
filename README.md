...
npm run tauri dev
```
初回画面にClient IDを貼り、「Spotifyに接続」を押します。ブラウザで許可すると表示が始まります。再生中は約0.4秒、停止中や再生曲がない場合も約1秒間隔で曲を確認します。
...
- マウスを置く: 表示設定と閉じるボタンを表示
- 歯車ボタン: スライド時間、背景ぼかし、文字サイズを調整

曲が変わると、新しいジャケットを先読みしてから短いスライドとフェードで表示を切り替えます。横幅変更中は文字スクロールを止め、リサイズ終了後に必要な場合だけ先頭から再開します。
表示設定は変更と同時にPC内へ保存され、次回起動時にも引き継がれます。「初期値に戻す」で標準設定へ戻せます。
